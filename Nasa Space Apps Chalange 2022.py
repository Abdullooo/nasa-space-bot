...
def info(message):
    global интересные_текст, интересные_фото
    count = seen_facts[message.chat.id] if message.chat.id in seen_facts.keys() else 1

    if count >= len(интересные_текст):
        count = 1
    ...
    seen_facts[message.chat.id] = count + 1


@bot.callback_query_handler(func=lambda call: call.data == "Ещё факт")
def more_info(call):
    global интересные_текст, интересные_фото
    count = seen_facts[call.message.chat.id] if call.message.chat.id in seen_facts.keys() else 1
    ...
    seen_facts[call.message.chat.id] = count + 1
...
def top_rating(message: types.Message):
    sorted_rating = dict(sorted(scores.items(), key=lambda x: x[1], reverse=True))
